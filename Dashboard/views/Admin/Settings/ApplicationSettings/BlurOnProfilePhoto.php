<?php 
$page="BlurOnProfilePhoto";     
include_once("views/Admin/Settings/ApplicationSettings/settings_header.php");  
                if (isset($_POST['BtnSave'])) {
                    
                    $target_dir = "uploads/";
                    if (!is_dir('uploads/BlurOnProfilePhoto/')) {
                        mkdir('uploads/BlurOnProfilePhoto/', 0777, true);
                    }
                    $err=0;
                    ...
                    if (isset($_FILES['File']['name']) && strlen(trim($_FILES['File']['name']))>0) {
                        ...
                        $BlurOnProfilePhoto = time().$_FILES["File"]["name"];   
                        if (!(move_uploaded_file($_FILES["File"]["tmp_name"],'uploads/BlurOnProfilePhoto/'. $BlurOnProfilePhoto))) {
                            $err++;
                            echo "Sorry, there was an error uploading your file.";
                        } else {
                            $_POST['File']= $BlurOnProfilePhoto;
                        }
                    ...
                    if ($err==0) {
                        ...
                        $res =$webservice->getData("Admin","UpdateBlurOnProfilePhoto",$_POST);   
...
$data  = $response['data']['BlurOnProfilePhoto'];              
            ?>
<script>
$(document).ready(function () {
    $("#Padding").blur(function () {
        if ($("#Padding").val()<= 0 && $("#Padding").val()>= 50) {                                                                 
   ...
   $("#Blur").blur(function () {
        if ($("#Blur").val()<= 0 && $("#Blur").val()>= 50) {                                                                 
            $("#ErrBlur").html("Please Enter Blur");  
        }else{
            $("#ErrBlur").html("");  
        }
   });
...
function SubmitProfile() { 
     ...
     $('#ErrBlur').html(""); 
      ...
      if(IsNonEmpty("Blur","ErrBlur","Please Enter Blur")){
        if (!( parseInt($('#Blur').val())>=0 && parseInt($('#Blur').val())<=50))    {
           $("#ErrBlur").html("Please enter below 50px") ;
           ErrorCount++;
        }
      }
...
<div class="col-sm-10 rightwidget">
    <form method="post" action="" enctype="multipart/form-data" onsubmit="return SubmitProfile();"> 
    <div class="">
                ...
                <div class="card-body">
                    <div >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                            <img src="<?php echo AppUrl;?>uploads/BlurOnProfilePhoto/<?php echo $data['ParamA'];?>"><br>
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Blur<span id="star">*</span></label>
                            <div class="col-sm-4">
                                <div class="input-group">
                                    <input type="text" required class="form-control" id="Blur" name="Blur" Placeholder="0" value="<?php echo (isset($_POST['Blur']) ? $_POST['Blur'] : $data['CodeDescription']);?>">
                                    <div class="input-group-addon">Px</div>
                                </div>
                                <span class="errorstring" id="ErrBlur"><?php echo isset($ErrBlur)? $ErrBlur : "";?></span>
                            </div>
                        </div>
                        <?php// } ?>
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-2">
                                <a href="javascript:void(0)" onclick="ConfirmSaveBlurOnProfilePhoto()" class="btn btn-primary mr-2" style="font-family:roboto">Save </a>
                                <input type="submit" name="BtnSave" id="BtnSave" style="display: none;">
                            </div>
                            <!--<div class="col-sm-3">
                                <a href="<?php echo AppUrl;?>Staffs/ViewBlurOnProfilePhoto">View</a>
                            </div>-->
                        </div>
                    </div>
                </div>
        ...
        <div class="col-4" id="PreviewDiv">
             <div class="card">
                <div class="card-body">
                    ...
                    <img src="../../im.php?p=<?php echo $data['ParamD'];?>&stamp=uploads/BlurOnProfilePhoto/<?php echo $data['ParamA'];?>&src=assets/images/sample_photo.png&h=<?php echo $data['ParamB'];?>&v=<?php echo $data['ParamC'];?>" style="width: 100%;">
                </div>
             </div>
        </div>
        <?php // } ?>
...
<script>
function ConfirmSaveBlurOnProfilePhoto(){
            if (SubmitProfile()) {
            ...
            var content =   '<div class="modal-header">'
                            ...
                            + '<div class="modal-footer">'
                                ...
                                + '<button type="button" class="btn btn-primary" name="BtnSaveProfile" class="btn btn-primary" onclick="GetTxnPasswordSaveBlurOnProfilePhoto()" style="font-family:roboto">Continue</button>'
                            + '</div>';                                                                                               
    ...
    function GetTxnPasswordSaveBlurOnProfilePhoto () {
        ...
        var content =  '<div class="modal-header">'
                      ...
                      + '</div>'
                        + '<div class="modal-footer">'
                            + '<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>&nbsp;&nbsp;'
                            + '<button type="button" onclick="SaveBlurOnProfilePhoto()" class="btn btn-primary" >Continue</button>'
                        + '</div>';
        $('#DeleteNow_body').html(content);            
    }
    function SaveBlurOnProfilePhoto() {
